...
def get_data_for_stan(y):
    ...
    assert len(y) > 0, "y must have positive length"
...
def estimate_R(
    ...
    assert (
        ...
        and isinstance(num_iter, int)
        and isinstance(num_warmup, int)
    ), "num_chains, num_iter, and num_warmup must be integers"

    assert (
        num_chains > 0 and num_iter > 0 and num_warmup > 0
    ), "num_chains, num_iter, and num_warmup must be positive"

    assert len(sig_levels) >= 1 and all(
        isinstance(x, int) for x in sig_levels
    ...
    for aa in sig_levels:
        ...
        lb = np.maximum(
            1 + 1 / gamma * np.percentile(fit_res["mu"], axis=0, q=aa / 2.0), 0.0
        )
    ...
    df_conv_stats = df_conv_stats.loc[
        mask,
    ]
    res["n_eff_pct"] = df_conv_stats["n_eff"].min() / float(
        num_chains * (num_iter - num_warmup)
    )
...
def simulate_AR1(rho, sigma, T, shocks=None):
    ...
    """
    assert rho > -1 and rho < 1, "Persistence parameter should be in (-1, 1)."

    if shocks is None:
        shocks = np.random.randn(1, T).flatten() * sigma
        shocks[0] = np.random.randn(1, 1) * sigma / np.sqrt(1 - rho ** 2)
    return {"shocks": shocks, "x": scipy.signal.lfilter([1], [1, -rho], shocks)}
