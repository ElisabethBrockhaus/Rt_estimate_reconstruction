...
DE_REGION_CODES = {v: k for k, v in DE_REGION_NAMES.items()}
...
def get_data_DE(run_date) -> pandas.DataFrame:
    ...
    # _log.warning(
    #     "Missing test counts for %s", str(case_regions.difference(test_regions))
    # )

    # df_result = pandas.merge(
    #     df_positives, df_testcounts, left_index=True, right_index=True, how="outer"
    # ).sort_index()
    df_result = df_positives.sort_index()
...
def get_positives_DE(run_date) -> pandas.DataFrame:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if run_date.date() == pandas.Timestamp(2021, 7, 10):
            ...
            encoding = "utf-8"
        elif run_date.date() < pandas.Timestamp(2021, 2, 7):
            ...
            print("Downloading German data from %s", release_url)
            with open(fp_tempfile, "wb") as file:
                file.write(requests.get(release_url).content)
            encoding = "utf-8"
        elif run_date.date() == datetime.datetime.utcnow().date():
            # EB:
            # release_url = "https://www.arcgis.com/sharing/rest/content/items/f10774f1c63e40168479a1feb6c7ca74/data"
            # _log.info("Downloading German data from %s", release_url)
            # with open(fp_tempfile, "wb") as file:
            #     file.write(requests.get(release_url).content)
            # encoding = "utf-8"
            ...
            anon_gis = GIS()
            features = (
                anon_gis.content.get("dd4580c810204019a7b8eb3e0b329dd6")
                .tables[0]
                .query()
            )
            ...
            print("Data version not available. Choose other run_date.")
        if CSV_SAVEPATH:
    ...
    df.Meldedatum = pandas.to_datetime(df.Meldedatum, format="%Y/%m/%d %H:%M:%S")
    ...
    df_merged.to_csv(
        "D:/EllasDaten/Uni/Wirtschaftsingenieurwesen/6Semester/Bachelorarbeit/Code/Rt_estimate_reconstruction/rtlive/rtlive-global/data/rtlive_data.csv"
    )
...
def get_testcounts_DE(run_date, take_latest: bool = True) -> pandas.DataFrame:
    ...
    if not fp_testcounts:
        raise FileNotFoundError(
            f"No testcounts file found in {dp_testcounts} for {date_str}"
        )
    ...
    df_merged = df_merged.reindex(
        pandas.MultiIndex.from_product(
            (df_merged.index.levels[0], df_merged.index.levels[1])
        )
    )
    return df_merged
...
def forecast_DE(
    ...
    (
        df["predicted_new_tests_raw"],
        results,
    ) = preprocessing.predict_testcounts_all_regions(df, "DE")
    ...
    for region in numpy.unique(df.index.get_level_values("region")):
        ...
        df.loc[
            pandas.IndexSlice[region, list(sfs.index)], "scaling_factor"
        ] = sfs.to_numpy()
...
def calculate_daily_scaling_factors(
    *, forecasted_daily_tests: pandas.Series, sparse_reported_totals: pandas.Series
) -> pandas.DataFrame:
...
def estimate_test_percentages_for_regions(df: pandas.DataFrame) -> pandas.Series:
    ...
    last_date_with_testcounts_for_all_regions = (
        rows_with_testcounts.groupby("region").tail(1).reset_index()["date"].min()
    )

    # select the last 7 days up to the latest testcount data point
    last_week_of_testcounts = slice(
        last_date_with_testcounts_for_all_regions - pandas.Timedelta("6D"),
        last_date_with_testcounts_for_all_regions,
    )

    # Then calculate the sum of tests one week up to that date
    testcounts_in_last_daily_data = (
        df.new_tests.xs(last_week_of_testcounts, level="date").groupby("region").sum()
    )
    ...
    for file in DATA_DIR.iterdir():
        if "Nowcasting" in str(file) and date_str in str(file):
            ...
            data_rki.index = pandas.to_datetime(
                data_rki.index,
                dayfirst=isinstance(data_rki.index[0], str)
                and ".2020" in data_rki.index[0],
            )
            if isinstance(data_rki["r4"][10], str) and "," in data_rki["r4"][10]:
                ...
                data_rki = data_rki.apply(
                    lambda col: [float(str(v).replace(",", ".")) for v in col]
                )

    result = {}
    if data_rki is not None:
        params = {"Rt": ("r4", "$R_t$", "green"), "Rt_7": ("r7", label_week, "orange")}
